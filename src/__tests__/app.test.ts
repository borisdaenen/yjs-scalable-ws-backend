...
export const testSingleDoc = async (tc: t.TestCase) => {
  ...
  const token = faker.internet.password({ length: 42 });
  const doc = new Y.Doc();
  ...
  const wsp = new NoBCWebsocketProvider(wsUrl(), id, doc, {
    params: { token },
...
export const testMultipleDocs = async (tc: t.TestCase) => {
  ...
  const id = faker.string.uuid();
  const token1 = faker.internet.password({ length: 42 });
  const token2 = faker.internet.password({ length: 42 });
  const token3 = faker.internet.password({ length: 42 });
  ...
  const wsp1 = new NoBCWebsocketProvider(wsUrl(), id, doc1, {
    params: { token: token1 },
    WebSocketPolyfill: WebSocket as any,
  });
  const wsp2 = new NoBCWebsocketProvider(wsUrl(), id, doc2, {
    params: { token: token2 },
    WebSocketPolyfill: WebSocket as any,
  });

  const words = range(0, 3).map(() => faker.word.sample());
  ...
  const wsp3 = new NoBCWebsocketProvider(wsUrl(), id, doc3, {
    params: { token: token3 },
    WebSocketPolyfill: WebSocket as any,
  });
  await new Promise<void>((resolve) => setTimeout(resolve, 100));
...
export const testRWAndRConnections = async (tc: t.TestCase) => {
  ...
  const id = faker.string.uuid();
  const rwToken = faker.internet.password({ length: 42 });
  const rToken = faker.internet.password({ length: 42 });
  ...
  const rwWsp = new NoBCWebsocketProvider(wsUrl(), id, rwDoc, {
    params: { token: rwToken },
    WebSocketPolyfill: WebSocket as any,
  });

  const rwItems = rwDoc.getArray("items");
  range(0, 5).forEach(() => rwItems.push([faker.word.sample()]));

  const rWsp = new NoBCWebsocketProvider(wsUrl(), id, rDoc, {
    params: { token: rToken },
    WebSocketPolyfill: WebSocket as any,
  });
  await new Promise<void>((resolve) => setTimeout(resolve, 100));
  ...
  range(0, 3).forEach(() => rwItems.push([faker.word.sample()]));
  await new Promise<void>((resolve) => setTimeout(resolve, 100));
